...
# Error_VA_DEPARTAMENTO             = DATA[grepl("[[:punct:]]", DATA$DEPARTAMENTO), c(Identificador)]
...
# Error_VA_MUNICIPIO                = DATA[grepl("[[:punct:]]", DATA$MUNICIPIO), c(Identificador)]
Error_VA_CODIGO_MUNICIPIO_DANE    = DATA[grepl("[[:punct:]]", DATA$`CODIGO MUNICIPIO DANE`), c(Identificador,"CODIGO MUNICIPIO DANE")]
...
Error_VA_TIPO_DOCUMENTO           = DATA[!DATA$`TIPO DOCUMENTO` %in% c("Cédula de Ciudadanía","Cédula de Extranjería","Documento Nacional de Identidad (DNI) del país de origen","Permiso especial de permanencia (PEP) para ciudadanos venezolanos","Registro Civil","Tarjeta de Identidad"), c(Identificador)]
Error_VA_NUMERO_DOCUMENTO         = DATA[grepl("[[:punct:]]", DATA$`NUMERO DOCUMENTO`) | is.na(DATA$`NUMERO DOCUMENTO`) | DATA$`NUMERO DOCUMENTO` %in% "" | !between(nchar(DATA$`NUMERO DOCUMENTO`),4,12), c(Identificador)]
